...
void copy_ind(individual *ind1, individual *ind2)
{
    ...
    // Copiar secuencias
    if (!ind2->num_seqs) ind2->num_seqs = (int *)malloc(pi->num_employees * sizeof(int));
    if (!ind2->seq_start_days) ind2->seq_start_days = (int **)malloc(pi->num_employees * sizeof(int *));
    if (!ind2->seqs) ind2->seqs = (ssequence ***)malloc(pi->num_employees * sizeof(ssequence **));

    for (i = 0; i < pi->num_employees; i++) {
        if (!ind2->seq_start_days[i]) ind2->seq_start_days[i] = (int *)malloc(pi->horizon_length * sizeof(int));
        if (!ind2->seqs[i]) ind2->seqs[i] = (ssequence **)malloc(pi->horizon_length * sizeof(ssequence *));
        ind2->num_seqs[i] = ind1->num_seqs[i];
        for (j = 0; j < ind1->num_seqs[i]; j++) {
            ind2->seqs[i][j] = ind1->seqs[i][j];  // shallow copy: mismo puntero
